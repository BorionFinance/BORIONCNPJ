...
scanIntoForm=async function(modal,file,type){const progress=$('[data-scan-progress]',modal);progress.hidden=false;const result=await ocrFile(file,msg=>progress.textContent=msg);if(type==='boleto'){const found=extractBoleto(result.text,result.barcode);if(found.linhaDigitavel)$('[name="linhaDigitavel"]',modal).value=found.linhaDigitavel;if(found.valor)$('[name="valor"]',modal).value=found.valor;if(found.vencimento)$('[name="vencimento"]',modal).value=found.vencimento;const val=found.linhaDigitavel?validateBoletoLine(found.linhaDigitavel):null;progress.innerHTML=found.linhaDigitavel?`Código localizado · confiança OCR ${Math.round(result.confidence)}% · ${val.valid?'válido':'confira os dígitos'}.`:`Não encontrei a linha completa · confiança OCR ${Math.round(result.confidence)}%. O arquivo continuará anexado.`}else{const found=extractCheque(result.text);if(found.numero)$('[name="numero"]',modal).value=found.numero;if(found.valor)$('[name="valor"]',modal).value=found.valor;if(found.data)$('[name="dataEmissao"]',modal).value=found.data;if(found.cmc7)$('[name="cmc7"]',modal).value=found.cmc7;progress.textContent=`Leitura concluída · confiança ${Math.round(result.confidence)}%. Confira número, CMC7, valor e data antes de salvar.`}};
...
renderConfig=function(){const root=$('#page-config');if(!root)return;const client=Drive.clientId(),rootId=App.state.settings.rootFolderId||'';root.innerHTML=`<div class="config-section drive-primary"><h3>Google Drive é o armazenamento oficial</h3><p>Ao entrar, o Borion procura ou cria a pasta <b>Borion CNPJ</b> na conta Google e salva nela o JSON atual, backups, fotos de cheques e fotos/PDFs de boletos. O navegador mantém somente uma cópia temporária para segurança.</p><div class="status-line"><span class="sync-dot ${App.drive.connected?'ok':'error'}"></span><div><b>${App.drive.connected?'Conectado como '+esc(App.user?.email||''):'Não conectado'}</b><small>${App.drive.connected?'Pasta raiz: '+esc(rootId):'Entre com o Google para criar a pasta'}</small></div></div></div><div class="config-section"><h3>Google OAuth</h3><div class="form-grid"><div class="field full"><label>Google OAuth Client ID</label><input id="cfg-client-id" value="${esc(client)}" placeholder="000000000000-xxxx.apps.googleusercontent.com"></div></div><button class="btn btn-primary" onclick="Borion.saveGoogleConfig()">Salvar configuração</button>${App.drive.connected?'<button class="btn btn-outline" onclick="Borion.reconnectGoogle()">Trocar conta Google</button>':''}</div><div class="config-section"><h3>Pasta oficial</h3><p>Deixe vazio para criar automaticamente na conta logada. Para você e seu pai usarem a mesma base, compartilhe a pasta e informe o mesmo link/ID nos dois computadores.</p><div class="form-grid"><div class="field full"><label>Link ou ID da pasta Borion CNPJ</label><input id="cfg-root-id" value="${esc(rootId)}" placeholder="criação automática"></div><div class="field"><label>Nome da empresa</label><input id="cfg-company" value="${esc(App.state.settings.companyName||'Borion CNPJ')}"></div><div class="field"><label>Dias de antecedência</label><input id="cfg-warning" type="number" min="1" max="90" value="${esc(App.state.settings.warningDays||7)}"></div><div class="field"><label>Qualidade das fotos (0.60 a 0.95)</label><input id="cfg-image-quality" type="number" min="0.60" max="0.95" step="0.01" value="${esc(App.state.settings.imageQuality||.86)}"></div><div class="field"><label>Tamanho máximo da imagem</label><input id="cfg-image-max" type="number" min="1200" max="5000" step="100" value="${esc(App.state.settings.maxImageDimension||2400)}"></div></div><label class="checkbox-row"><input id="cfg-auto-sync" type="checkbox" ${App.state.settings.autoSync?'checked':''}> Sincronizar automaticamente depois de cada alteração</label><div style="height:12px"></div><button class="btn btn-primary" onclick="Borion.saveDriveConfig()">Salvar preferências</button>${App.drive.connected?'<button class="btn btn-outline" onclick="Borion.prepareDrive()">Criar/verificar todas as pastas</button>':''}</div><div class="config-section"><h3>Estrutura automática</h3><div class="code-path">Borion CNPJ / Sistema / Dados / current.json.borion<br>Borion CNPJ / Sistema / Backups / 2026 / 08 - Agosto<br>Borion CNPJ / Cheques / 2026 / 08 - Agosto<br>Borion CNPJ / Boletos / 2026 / 08 - Agosto</div></div>`};
saveDriveConfig=async function(){App.state.settings.rootFolderId=Drive.folderIdFromInput($('#cfg-root-id')?.value||'');App.state.settings.companyName=$('#cfg-company')?.value.trim()||'Borion CNPJ';App.state.settings.warningDays=Math.max(1,Math.min(90,Number($('#cfg-warning')?.value)||7));App.state.settings.autoSync=$('#cfg-auto-sync')?.checked;App.state.settings.imageQuality=Math.max(.6,Math.min(.95,Number($('#cfg-image-quality')?.value)||.86));App.state.settings.maxImageDimension=Math.max(1200,Math.min(5000,Number($('#cfg-image-max')?.value)||2400));localStorage.setItem('borion_cnpj_root_folder_id',App.state.settings.rootFolderId);App.drive.rootId=App.state.settings.rootFolderId;await saveLocal();renderAll();toast('Configurações salvas.');if(App.drive.connected)scheduleSync()};
...
window.Borion={
  ...
  organizePhotos:()=>Organizer.open(),exportBackup,importBackup,exportTeamKey,importTeamKey,saveGoogleConfig,saveDriveConfig,prepareDrive,reconnectGoogle,driveBackup:()=>Drive.manualBackup()
};
