...
assert(index.includes('1.0.23'),'index.html não está na versão 1.0.23');
assert(config.includes("version: '1.0.23'"),'config.js não está na versão 1.0.23');
...
assert(app.includes('Drive._pullOnceV118'),'carregamento seguro v1.0.23 ausente');
assert(app.includes('Drive.writeCanonicalV118'),'gravação canônica v1.0.23 ausente');
...
assert(app.includes('lote-photo-cell'),'v1.0.23: fotos por cheque no lote ausentes');
assert(app.includes('data-org-file-drop'),'v1.0.23: drop externo em Fotos em massa ausente');
assert(app.includes('cheque-editado-com-foto-v122'),'v1.0.23: confirmação local do anexo na edição ausente');
assert(app.includes('fotos-em-massa-v122'),'v1.0.23: persistência das fotos em massa ausente');
...
assert(sw.includes("borion-cnpj-v1.0.23"),'cache PWA não está em 1.0.23');

assert(app.includes("const V123_VERSION='1.0.23'"),'v1.0.23: camada transacional ausente');
assert(app.includes('v123HasActiveEditor'),'v1.0.23: trava de atualização durante edição ausente');
assert(app.includes('Drive._pullOnceV123'),'v1.0.23: pull protegido contra concorrência ausente');
assert(app.includes("cheque-editado-imediato-v123"),'v1.0.23: edição imediata de cheque ausente');
assert(app.includes("fotos-em-massa-todas-associacoes-v123"),'v1.0.23: salvamento integral das associações ausente');
assert(index.includes('js/app.js?v=1.0.23'),'index.html não força app.js 1.0.23');
assert(index.includes("service-worker.js?v=1.0.23"),'index.html não força service worker 1.0.23');

if(!process.exitCode) console.log('BORION CNPJ 1.0.23 validado com sucesso.');
