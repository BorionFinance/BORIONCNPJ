...
assert(index.includes('1.0.10'),'index.html não está na versão 1.0.10');
assert(config.includes("version: '1.0.10'"),'config.js não está na versão 1.0.10');
...
assert(index.includes('borion-cnpj-v110-emblem.png'),'emblema oficial v109 não referenciado');
assert(index.includes('borion-cnpj-v110-apple-touch-icon.png'),'ícone mobile oficial v109 não referenciado');
assert(fs.existsSync('borion-cnpj-v110-icon-192.png'),'ícone 192 oficial ausente');
assert(fs.existsSync('borion-cnpj-v110-icon-512.png'),'ícone 512 oficial ausente');
assert(fs.readFileSync('service-worker.js','utf8').includes("borion-cnpj-v1.0.10"),'cache PWA não está em 1.0.10');

if(!process.exitCode) console.log('BORION CNPJ 1.0.10 validado com sucesso.');
