...
assert(index.includes('1.0.22'),'index.html não está na versão 1.0.22');
assert(config.includes("version: '1.0.22'"),'config.js não está na versão 1.0.22');
...
assert(app.includes('Drive._pullOnceV118'),'carregamento seguro v1.0.22 ausente');
assert(app.includes('Drive.writeCanonicalV118'),'gravação canônica v1.0.22 ausente');
...
assert(app.includes('lote-photo-cell'),'v1.0.22: fotos por cheque no lote ausentes');
assert(app.includes('data-org-file-drop'),'v1.0.22: drop externo em Fotos em massa ausente');
assert(app.includes('cheque-editado-com-foto-v122'),'v1.0.22: confirmação local do anexo na edição ausente');
assert(app.includes('fotos-em-massa-v122'),'v1.0.22: persistência das fotos em massa ausente');
...
assert(sw.includes("borion-cnpj-v1.0.22"),'cache PWA não está em 1.0.22');
if(!process.exitCode) console.log('BORION CNPJ 1.0.22 validado com sucesso.');
